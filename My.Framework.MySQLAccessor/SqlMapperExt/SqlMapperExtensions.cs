...
namespace My.Framework.MySQLAccessor
{
    ...
    public static partial class SqlMapperExtensions
    {
        ...
        private static List<PropertyInfo> ExtentionPropertiesCache(Type type)
        {
            ...
            var computedProperties = TypePropertiesCache(type).Where(p => p.GetCustomAttributes(true).Any(a => a is ExtentionAttribute)).ToList();
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            if (!GetQueries.TryGetValue(type.TypeHandle, out string sql))
            {
                ...
                sql = $"select * from `{name}` where `{key.Name.ToColName()}` = @id";
        ...
        public static bool Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var computedProperties = ExtentionPropertiesCache(type);
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            var keyAndExplicitKey = keyProperties.Union(explicitKeyProperties).ToList();
            var computedProperties = ExtentionPropertiesCache(type);
            var nonIdProps = allProperties.Except(keyAndExplicitKey.Union(computedProperties)).ToList();
            ...
            sb.Append(" where ");
            for (var i = 0; i < keyAndExplicitKey.Count; i++)
            {
                var property = keyAndExplicitKey[i];
                adapter.AppendColumnNameEqualsValue(sb, property.Name);  //fix for issue #336
                if (i < keyAndExplicitKey.Count - 1)
                    sb.Append(" and ");
            }
...
public partial class MySqlAdapter : ISqlAdapter
{
    ...
    public int Insert(IDbConnection connection, IDbTransaction transaction, int? commandTimeout, string tableName, string columnList, string parameterList, IEnumerable<PropertyInfo> keyProperties, object entityToInsert)
    {
        ...
        var row = connection.Execute(cmd, entityToInsert, transaction, commandTimeout);
    ...
    {
        sb.AppendFormat("`{0}`", columnName.ToColName());
    }
    ...
    {
        sb.AppendFormat("`{0}` = @{1}", columnName.ToColName(), columnName.ToLower());
    }
}
